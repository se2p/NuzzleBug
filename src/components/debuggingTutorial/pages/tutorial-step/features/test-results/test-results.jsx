...
const TestResults = props => {
    ...
    return (
        <div className={css.container}>
            <img
                src={bubbleIndicatorGreenDark}
                style={{display: 'none'}}
                alt={'preload'}
            />
            <img
                src={bubbleIndicatorGreenLight}
                style={{display: 'none'}}
                alt={'preload'}
            />
            <img
            ...
            <div className={css.whiteBoxTop}>
                <div className={css.bubbleContainer}>
                    ...
                    <img
                        src={curTestDetails === '' ? owl : owlDown}
                        alt="Picture of Euli"
            ...
            <div className={css.testNavigationWrapper}>
                <div
                    className={css.testLeftArrow}
                    onClick={() => onDecreaseTestPageIndex()}
                ...
                <div
                    className={css.testRightArrow}
                    onClick={() => onIncreaseTestPageIndex()}
                    style={{visibility: showRightArrow ? 'visible' : 'hidden'}}
                >
                    <div
                        className={css.nextTestIcon}
...
const renderResponse = ({projectLoadingState, nextStep, testResults, handleTestStart, guiMessages, step, showTestAgainBubble, hideTestAgainBubble}) => {
    ...
    if (projectLoadingState === 'TEST' || projectLoadingState === 'TEST_PAUSE') {
        return (
            <div style={{width: '100%'}}>
                ...
                {projectLoadingState !== 'TEST_PAUSE' && <span className={css.loader} />}
            </div>
        );
    }
    ...
    if (showTestAgainBubble) {
        return (
            <div
                className={`${css.helpBubble} ${(projectLoadingState !== 'TEST') ? '' : css.selected}`}
                onClick={() => {
                    if (projectLoadingState !== 'TEST') handleTestStart(); hideTestAgainBubble();
                }}
...
const createTestElement = (passed, e, testElementNumber, isCurrentStep, projectLoadingState, curTestDetails, setCurTestDetails, guiMessages, openHelpPage, tutorialMessages) => {
    ...
    if (curTestDetails === e.testId) {
        return (
            <div
                key={e.testId}
                className={css.testElementContainer}
                style={{width: '450px'}}
            >
                <div
                    className={(projectLoadingState === 'TEST' && e.result === 'running') ? css.testElementHeaderAnimation : css.testElementHeader}
                    style={{backgroundColor: headerBgColor, height: '60px'}}
                >
                    <div className={css.testElementTitleExtended}>
                        <img
                            src={iconSrc}
                            style={{width: '43px', marginRight: '10px'}}
                            className={css.testElementIcon}
                            alt={'ResultIcon'}
                            draggable={false}
                        />
                ...
                <p
                    className={css.testElementText}
                    key={e.testId}
                >{tutorialMessages[`test${e.testId}`].description}</p>
                {passed ?
                    <div
                        className={css.buttonWrapper}
                        onClick={() => setCurTestDetails('')}
                    >
                    ...
                    <div className={css.buttonContainer}>
                        ...
                        {tutorialConfig.classic.hintSystemEnabled &&
                            <div
                                ...
                                onClick={() => {
                                    if (projectLoadingState !== 'TEST' && projectLoadingState !== 'TEST_PAUSE') openHelpPage();
                                }}
                            >
    ...
    return (
        ...
        >
            ...
            <div
                className={css.buttonWrapper}
                onClick={() => setCurTestDetails(e.testId)}
            >
                <div className={buttonStyle}>
                    {msg.details_button}
                </div>
