    ...
    return (
        <div className={css.helpPageQuiz}>
            ...
            {questionIndex === 0 && <div className={css.qaOptions} role="group" aria-label="Antwortoptionen">
                {spriteUrls.length === 0 ? (
                    <button className={css.qaOption} type="button" disabled>
                        <span className={css.pill}>Lade Sprites…</span>
                    </button>
                ) : (
                    <>
                        {spriteUrls.map((url, i) => (
                            <button key={url ?? i} className={css.qaOption} type="button" onClick={() => {increaseQuestionIndex()}}>
                                <FitToWidth>
                                    <img src={url} alt="" />
                                </FitToWidth>
                            </button>
            ...
            {questionIndex === 1 && <div className={css.centerRow}>
                <div className={css.gridOptions} role="group" aria-label="Antwortoptionen">
                    {blockTypes.map(item => (
                        <button key={item[1]} className={css.blockTypeOption} type="button" onClick={() => {increaseQuestionIndex()}}>
                            <div className={css.circle} style={{backgroundColor:item[1]}}/>
                            <span>{item[0]}</span>
                        </button>
                    ))}
                </div>
            ...
            {questionIndex === 2 && <div className={css.verticalOptions} role="group" aria-label="Antwortoptionen">
                {kognitionOptions.map(text => (
                    <button className={`${css.qaOption} ${css.qaOptionTall}`} type="button" onClick={() => {increaseQuestionIndex()}}>
                        {text}
                    </button>
                ))}
            </div>}
